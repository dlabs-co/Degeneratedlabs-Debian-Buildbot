...
launch_qemu(){ arch=$1; shift; qemu $@ images/debian_${version}_${arch}_standard.qcow2; }
...
prepare_image(){ 
    ...
    [[ ! -f images/debian_${version}_${1}_standard.qcow2 ]] && { 
	    wget $image_url -O images/debian_${version}_${1}_standard.qcow2 &> $log && {
            ...
            echo "wget \"$script_url\" -O /etc/init.d/dlabs_builder; chmod +x /etc/init.d/dlabs_builder; update-rc.d dlabs_builder default; halt;"
...
for arch in $archs; do build_package_and_iso $arch & done # Build packages and isos in parallel... DANGEROUS. make sure to not build it for more than two archs as a time if you don't have a huge computer
